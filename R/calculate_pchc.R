...
#' `eq5d::pchc()` function providing methods for [EQ5D][as_eq5d] objects.
#'
# -------------------------------------------------------------------------
...
calculate_pchc <- function(pre, post, no.problems = TRUE, by.dimension = FALSE) {
    ...
    # check inputs
    .assert_class(pre, "EQ5D")
    .assert_class(post, "EQ5D")
    ympes::assert_bool(no.problems)
    ympes::assert_bool(by.dimension)
    ...
    # check each input is for only one survey
    ...
    if (by.dimension) {
        reverse_lookup <- stats::setNames(names(lookup), c("MO", "SC", "UA", "PD", "AD"))
