...
lme4::lmer(.value ~ (1 + acute | respondentID) + surveyID + sex + age + sex:age, data = dat) -> fit
...
seq(1, 1000) |> purrr::map(function(np_id) {
  ...
  lst <-
    tryCatch({
    lme4::lmer(.value ~ (1 + acute | respondentID) + surveyID +
             sex + age + sex:age, data = boot_dat) -> fit
  list(model = fit, np = as.numeric(np_id))
    }, error = function(c) { return(list())
    }, warning = function(c) { return(list()) })
}) |> purrr::keep(function(lst0) length(lst0) > 0) -> models
...
coeff_df |>
  dplyr::group_by(id) |>
  dplyr::summarise(quant = c(0.025, 0.5, 0.975),
                   value = quantile(Estimate, quant),
                   sig = all(value < 0) || all(value > 0)
                   ) |>
  tidyr::spread(quant, value) |>
  dplyr::ungroup() |>
  dplyr::mutate(id = as_term_factor(id)) -> table_df
```
...
summary(fit)
brms_fit <- fit
#qs::qsave(brms_fit, "~/Downloads/brms_fit.qs")
```
