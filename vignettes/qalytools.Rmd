...
- Coercion functions `as_eq5d3l()`, `as_eq5d5l()` and `as_eq5dy3l()` for EQ5D
data frame subclasses (EQ5D3L, EQ5D5L and EQ5DY3L).
...
and `available_valuesets()` functions which are wrappers around the functionality
of the underlying eq5d package.
...
wrapping functions from the eq5d package).
...
package, a more detailed analysis example can be found in the
[EQ5D Analysis article](https://timtaylor.github.io/qalytools/articles/example_analysis.html) on the package website.
